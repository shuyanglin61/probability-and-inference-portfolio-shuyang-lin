...
Sample size set to 10000:

```{r}
set.seed(114514)
R <- 10000
N <- 4
result <- t(data.frame(replicate(R, game(N))))
colnames(result) <- c('reveal', 'switch')
(
  part1A <- mean(ifelse(result[,'reveal'] == result[,'switch'] & result[,'reveal'] == 'goat', 1, 0))
)
```

**Answer**: The estimated probability is 0.3701.
...
```{r}
# answer to Part1 B
prop.test(part1A*R, R, conf.level = 0.99)
```

**Answer**: The confidence interval is between 0.3577034 and 0.3826695.
...
for(i in 1:8) {
  R <- 10000
  result <- data.frame(t(replicate(R, game(i+2))))
  colnames(result) <- c('reveal', 'switch')
  D[i] <- mean(result[,'switch'] == 'car') - mean(result[,'reveal']=='car')
}
```

```{r}
(
  part1C <- ggplot() + geom_line(aes(3:10,D)) + labs(x = 'N', y='D') + theme_classic()
)
```
...
**Solution 1**
50% of students are 0, and gamma distribution with shape 2 will not be 0.
Thus we need the 80th percentile of the remaining 50% for the answer
```{r}
(
  Part3C1 <- qgamma(0.8, 2, scale=2)
)
```

**Solution 2**

Simulation with 10000 students and 500 times:

```{r}
set.seed(114514)
(
  Part3C2<- mean(replicate(500, quantile(hospital_charges(10000), 0.9)))
)
```
...
```{r}
(
  Part3B <- ggplot() + geom_histogram(aes(replicate(10000,mean(hospital_charges(30))),y=..density..)) + labs(x = 'Class Average Yearly Hospital Charge with Size 30',y='Density') + theme_classic()
)
```
...
```{r}
(
  Part4C <- pnorm(104,80,15) - pnorm(70, 80, 15)
)
```
...
**Answer**: B = long 37 cm, A = male
P(A|B) = P(B|A)P(A)/(P(B|A)P(A) + P(B|-A)P(-A))
```{r}
(
  Part4B <- dnorm(37,40,3.4)*0.5 / (dnorm(37,40,3.4)*0.5 + dnorm(37,36,3.3)*0.5)
)
```
...
```{r}
femur_length <- 25:50
prob_male <- dnorm(femur_length, 40, 3.4)/(dnorm(femur_length, 40, 3.4) + dnorm(femur_length, 36, 3.3))
plot.new()
...
set.seed(114514)
(
  Part5C2 <- mean(hospital_charges(10000))
)
```
...
R <- 5000
for (i in 1:R){
  out[i] <- median(rlnorm(101,meanlog = 0,sdlog = 1))
}

ggplot() + geom_histogram(aes(out,..density..)) + labs(x = 'Sampling Median', y= 'Density') + theme_bw()
```
...
```{r}
ggplot() + stat_ecdf(aes(out), color = 'red') + geom_line(aes(seq(0.5,1.8,0.001), Fk(seq(0.5,1.8,0.001), 51, 101)), color = 'blue') + theme_bw() + labs(y='Fk')
```
...
for (i in 1:10000){
  ...
  out_25[i] <- quantile(sample, .25)
  out_50[i] <- quantile(sample, .5)
  out_75[i] <- quantile(sample, .75)
}
```

```{r}
(
  Part6A25 <- quantile(out_25, 1-0.05/2) - quantile(out_25, 0.05/2)
)
```

```{r}
(
  Part6A50 <- quantile(out_50, 1-0.05/2) - quantile(out_50, 0.05/2)
)
```

```{r}
(
  Part6A75 <- quantile(out_75, 1-0.05/2) - quantile(out_75, 0.05/2)
)
```

```{r}
(
  Part6Aplot <- ggplot() + geom_density(aes(out_25, color='25th')) + geom_density(aes(out_50, color = '50th')) + geom_density(aes(out_75, color='75th')) + labs(x = 'Quantiles', y = 'Density') + theme_classic() + scale_color_manual(name = 'Quantile', values = c('25th' = 'green', '50th' = 'blue', '75th' = 'red'))
)
```

**Answer**: The 25th quantiles of the distribution has the tighest 95% CI.
...
**Code**: In normal distribution , the MLE result is the same as the MM result.

```{r}
male_adult_ht <-  (d1 %>% filter(age > 18 , sex == "male"))$ht

mean_mle_male <- mean(male_adult_ht)
sd_mle_male <- sd(male_adult_ht)
(
  Part7C <- ggplot() + geom_histogram(aes(male_adult_ht, ..density..),fill='white', color = 'black', bins=20) + geom_line(aes(x = min(male_adult_ht):max(male_adult_ht), y = dnorm(min(male_adult_ht):max(male_adult_ht), mean_mle_male, sd_mle_male))) + theme_bw() + labs(x='Male Adult Height', y='Density')
)
```
...
```{r}
female_adult_bmi <- (d1 %>% filter(age > 18 , sex == "female"))$bmi

shape_mm_gamma <- mean(female_adult_bmi)^2/var(female_adult_bmi)
scale_mm_gamma <- var(female_adult_bmi)/mean(female_adult_bmi)

(
  Part7B <- ggplot() + geom_histogram(aes(female_adult_bmi, ..density..),fill='white', color = 'black', bins=20) + geom_line(aes(x = 0:max(female_adult_bmi), y = dgamma(0:max(female_adult_bmi), shape = shape_mm_gamma , scale = scale_mm_gamma))) + theme_bw() + labs(x='Female Adult BMI', y='Density')
)
```
...
```{r}
adult_scr <- (d1 %>% filter(age > 18 ))$SCr %>% na.omit

plot(density(adult_scr, bw='nrd0', adjust=1,kernel='gaussian'))
...
```{r}
bwt_smoke <- (bwt %>% filter(smoke == 1))$bwt %>% na.omit
...
```{r}
bwt_nosmoke <- (bwt %>% filter(smoke == 0))$bwt

out <- replicate(10000, mean(sample(bwt_smoke, length(bwt_smoke), replace = T))/mean(sample(bwt_nosmoke, length(bwt_nosmoke), replace = T)))
...
```{r}
(t1 <- prop.test(c(30, 35), c(100, 100)))
```

```{r}
(t2 <- prop.test(c(600,619), c(2000,2000)))
```

We cannot reject the null hypothesis in both studies.
...
The second study's CI is included in [-0.06, 0.06] while the first one not.
...
```{r}
(t3 <- prop.test(c(600 + 30, 619 + 35), c(600 + 30 + 1400 + 70, 619 + 35 + 65 + 1381)))
```

```{r}
ceiling(max(abs(t3[[6]][1]), abs(t3[[6]][2]))*100)
```
...
**Answer**: Conditional distribution has more information than marginal distribution does. With two random variables related, we could have a smaller variance, and thus the prediction error will be smaller. However, if they are not related, we know that P(A|B) = P(AB)/P(B) = P(A)*P(B)/P(B) = P(A), and the variance will be the same.
...
The p-value is lest than 0.05, and 95% CI doesn't include 0, so we are able to reject the null hypothesis and conclude that they are related.
